...
The pipeline API provides comprehensive entity resolution and taxonomy classification through a 6-stage plugin architecture.
...
class PipelineConfig(BaseModel):
    enabled_stages: set[int] = {1, 2, 3, 4, 5, 6}  # Stages to run (1-6)
...
class PipelineContext(BaseModel):
    ...
    qualified_entities: dict[str, QualifiedEntity] = {} # Stage 3 output
    canonical_entities: dict[str, CanonicalEntity] = {} # Stage 4 output
    labeled_statements: list[LabeledStatement] = []    # Stage 5 output
    taxonomy_results: dict[tuple, list[TaxonomyResult]] = {}  # Stage 6 output (multiple labels per statement)
...
Output of Stage 3 (Qualification).
...
Result of canonical matching in Stage 4.
...
class CanonicalMatch(BaseModel):
    ...
    match_method: str                    # identifier, name_exact, name_fuzzy, llm_verified
...
Output of Stage 4 (Canonicalization).

```python
class CanonicalEntity(BaseModel):
    entity_ref: str                      # Reference to original entity
    qualified_entity: QualifiedEntity    # Qualified entity from Stage 3
    canonical_match: Optional[CanonicalMatch]  # Canonical match if found
    fqn: str                             # Fully Qualified Name

    @classmethod
    def from_qualified(cls, qualified: QualifiedEntity, ...) -> "CanonicalEntity":
        """Create from QualifiedEntity."""
```
...
Output of Stage 6 (Taxonomy) classification.
