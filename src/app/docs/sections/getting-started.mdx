...
With embeddings support for semantic similarity and clustering:
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>transformers</td>
      <td>git (dev)</td>
      <td>Required, from <a href="https://github.com/huggingface/transformers">GitHub</a></td>
    </tr>
    <tr>
      <td>Pydantic</td>
      <td>2.0+</td>
    ...
    <tr>
      ...
      <td>2.2+</td>
      <td>Optional, for <code>[embeddings]</code></td>
    </tr>
  </tbody>
</table>
...
- **GPU**: ~2GB VRAM minimum. CUDA-enabled NVIDIA GPU recommended for production.
- **CPU**: ~4GB RAM. Functional but significantly slower. Use for development or low-volume processing.

The model uses bfloat16 precision on GPU (faster, less memory) and float32 on CPU.
